...
class MicropostSidebarTest < MicropostsInterface
  ...
  test "should use proper pluralization for zero microposts" do
    # 0個のマイクロポストを持つユーザーにログインするためにsetupを上書き
    user_with_zero_microposts = users(:malory)
    log_in_as(user_with_zero_microposts)
    get root_path
    assert_match "0 microposts", response.body
  end

  test "should use proper pluralization for one micropost" do
    # 1個のマイクロポストを持つユーザーにログインするためにsetupを上書き
    user_with_one_micropost = users(:lana)
    log_in_as(user_with_one_micropost)
    get root_path
